...
<?php
    ...
    if(isset($_POST['route_name'])) {    
        ...
        $routeResult = $bus->select($rows, $table, $join, $where , $order); 
    ...
    if(isset($_GET['route'])) {
        ...
        $where = 'trips.route_id = "'.$_GET['route'].'"';
        $order = '';
            
        $tripsResult = $bus->select($rows, $table, $join, $where , $order);
